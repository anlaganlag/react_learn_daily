...
class Movie extends Component {
  ...
  componentDidMount() {
    ...
    if (localStorage.getItem(`${movieId}`)) {
      ...
      let endpoint = `${API_URL}movie/${movieId}?api_key=${API_KEY}&language=zh-CN`;
    ...
    .then(result => {
      ...
      if (result.status_code) {
        ...
        this.setState({ movie: result }, () => {
          ...
          let endpoint = `${API_URL}movie/${movieId}/credits?api_key=${API_KEY}&language=zh-CN`;
          fetch(endpoint)
          .then(result => result.json())
          .then(result => {
  ...
  render() {
    ...
    return (
      <div className="rmdb-movie">
        ...
        : null }
        {!actors && !loading ? <h1>沒有找到電影</h1> : null }
